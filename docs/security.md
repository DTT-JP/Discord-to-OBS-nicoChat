...
| `/start` / `/auth` | **サーバーオーナー・管理者**、または **`/setup` で許可された**ロール／ユーザー（`AllowedPrincipalDB`） |
| `/session` | **グローバルBL以外**のユーザーが実行可能。ただし更新できるのは**自分が作成したセッション**のみ |
| `/secret` | **グローバルBL以外**のユーザーが実行可能。適用対象は、実行者本人のセッションと `secret` 許可中セッション |
