...
`/session` は **グローバルブラックリスト対象外**なら実行できますが、更新対象は **自分が作成したセッション**のみです。`/start` 時にも `secret` 許可/拒否を指定できます。

### 認証（`/auth`）

`/auth` は **DM ではなく**、**`/start` を実行したサーバーのチャンネル**で行ってください。オーバーレイ（`public/index.html`）の案内と同じです。
