...
ホームページはこちら [https://d2obs.dttjp.com/](https://d2obs.dttjp.com/)
...
- ニコニコ動画風スクロール表示（文字数ベースで速度調整・短いコメントは約4秒で横断）
- `? ... ?` メタデータによる色・サイズ・位置・フォント指定
- ゴシック体 / 明朝体のフォント切り替え（`?gothic?` / `?mincho?`）
...
公開BOTの規約:
- プライバシーポリシー: [https://d2obs.dttjp.com/privacy](https://d2obs.dttjp.com/privacy)
- 利用規約: [https://d2obs.dttjp.com/terms](https://d2obs.dttjp.com/terms)
...
| 属性 | 値の例 | 効果 |
|---|---|---|
| 色 | `red` `blue` `#ff8800` | テキスト色 |
| サイズ | `big` `medium` `small` | 文字サイズ |
| 位置 | `ue` `shita` | 上下固定（4秒）または省略で横流れ |
| フォント | `gothic` `mincho` | ゴシック体 / 明朝体（未指定はgothic） |

書式: `?属性1 属性2? 本文` — 属性の位置・順序は自由です。  
詳細は [docs/message-format.md](docs/message-format.md) を参照してください。
