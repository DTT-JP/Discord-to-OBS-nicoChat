...
- カスタム絵文字のインライン画像表示（サイズはテキストに連動）
- 改行・AA（アスキーアート）対応（空白・位置が崩れない）
- Discord書式（太字・斜体・下線・取り消し線・見出し）の反映
- `? ?` メタデータ書式による色・サイズ・位置・隠しコマンド指定（テキストのどこに書いてもOK）
- 上下固定表示モード（各コメント個別タイマー・消えても位置を詰めない）
- セッションごとの同時表示上限設定（1〜10000）・配信中のリアルタイム変更
- グローバルブラックリスト（Bot製作者専用・全サーバー共通遮断）
- 複数サーバーへの独立した対応（サーバーごとに設定が完全分離）
- ローカル運用・HTTPS外部公開の両対応（`PUBLIC_URL` で切り替え）
- CPU・メモリ使用率のリアルタイム監視（`/status`）
- 衝突判定によるコメントの重なり防止
...
# ── Bot製作者設定 ──────────────────────────────────
# /blacklist コマンドを実行できるユーザーのDiscord ID
...
上下固定コメントの特性として、各コメントは個別タイマーで管理されます。消えても他のコメントの位置は動かず、空きスロットに新しいコメントが入ります。
...
| コマンド | 効果 |
|---|---|
| `invisible` | コメントをオーバーレイに表示しない |
| `full` | コメントの臨界幅を変更 |
| `patissier` | コメント保持数の条件を変更 |
| `ender` | 改行リサイズを無効化してそのままのサイズで表示 |
| `_live` | コメントを半透過表示（複数重ねると濃くなる） |
| `ca` | ニコる機能によるコメント残存期間の延長を回避 |
...
テキスト ?shita green?             → 緑色・下部固定（末尾に書いてもOK）
?invisible? 内緒のメッセージ       → オーバーレイに表示されない
?_live? 薄いコメント               → 半透過表示
?ender? 改行してもリサイズしない    → 改行無効
...
複数組み合わせ可能です（例: `**__太字下線__**`）。

#### 改行・AA（アスキーアート）

Discordで Shift+Enter で改行して送信すると、OBSにも複数行でそのまま表示されます。行内の空白も保持されるため、全角スペースを使ったAAの位置合わせが崩れません。

#### カスタム絵文字・スタンプ

サーバー固有のカスタム絵文字（`<:name:id>`）とスタンプはOBS上にも画像として表示されます。絵文字のサイズはテキストのフォントサイズに連動します。Unicode絵文字（😊など）はテキストとしてそのまま表示されます。
...
| コマンド | 説明 |
|---|---|
| `/blacklist add [ユーザーID]` | ユーザーをグローバルブラックリストに追加（全サーバー共通で完全遮断） |
...
一部のコマンドは意図的に非公開となっています。`/help` コマンドで詳細を確認できます。
...
**セッションのスコープ分離**
複数サーバーに導入した場合、各サーバーの設定（許可ロール・ユーザー）はサーバーIDで完全に分離されます。あるサーバーの設定が別サーバーに影響することはありません。
...
OBSのブラウザソースを右クリック → **更新** を試してください。Node.jsコンソールに `[manager] 接続確立・コード生成` が出ているか確認してください。`HOST` と `PORT`（または `PUBLIC_URL`）の設定がOBSからアクセス可能なアドレスになっているか確認してください。
...
`/auth` で認証が完了しているか確認してください（OBSの認証コード画面が消えているはずです）。Node.jsコンソールに `[manager] auth_success 送信` が出ているか確認してください。ブラウザの開発者ツール（F12）でエラーが出ていないか確認してください。
...
`public/script.js` のバージョンを確認してください。`img.style.fontStyle = "normal"` が設定されているバージョンが必要です。

### 上下固定コメントが重なる

大きい文字（`big`）を使用している場合、スロットの高さ計算が正しく行われているか確認してください。コメントが消えた後に空きスロットに新しいコメントが正しく配置されるはずです。
