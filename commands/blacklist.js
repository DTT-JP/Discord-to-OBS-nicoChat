...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) =>
    sub
      .setName("add")
      .setDescription("ユーザーをグローバルブラックリストに追加します")
      .addStringOption((opt) =>
        opt
          .setName("user_id")
...
export async function execute(interaction) {
  ...
  if (sub === "add") {
    ...
    const added = await BlacklistDB.add(userId, interaction.user.id);
    if (!added) {
      return interaction.editReply({
        content: `⚠️ ユーザーID \`${userId}\` はすでにブラックリストに登録されています。`,
      });
    }

    return interaction.editReply({
      content: `🚫 ユーザーID \`${userId}\` をグローバルブラックリストに追加しました。\n全サーバーでコマンドへの反応とオーバーレイへの表示が遮断されます。`,
    });
  }
  ...
  if (sub === "remove") {
    const userId = interaction.options.getString("user_id", true).trim();
    const removed = await BlacklistDB.remove(userId);

    if (!removed) {
      return interaction.editReply({
        content: `⚠️ ユーザーID \`${userId}\` はブラックリストに登録されていません。`,
      });
    }
  ...
  // ── list ─────────────────────────────────────
  if (sub === "list") {
    const entries = BlacklistDB.findAll();
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({ text: "このリストは全サーバーに共通して適用されます" })
      .setTimestamp();
