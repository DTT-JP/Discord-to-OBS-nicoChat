        ...
        .setFields(
          {
            ...
            value: [
              ...
              "**テキストのどこに書いても認識されます。複数属性はスペース区切りで同時指定できます。**",
              "",
              "**色指定**",
              ...
              "",
              "**サイズ指定**",
              "`big`（大・約7行まで縮小なし）/ `medium`（中・約12行まで・デフォルト）/ `small`（小・約21行まで）",
              "Discord書式 `# テキスト`（H1〜H3、`-#`）でも指定可能",
              "",
              "**位置指定**",
              "`ue`（上固定・4秒）/ `shita`（下固定・4秒）",
              "指定なし → 画面を横断して流れます（短いコメントは約4秒、長いコメントはやや遅め）",
              "",
              "**フォント指定**",
              "`gothic`（ゴシック体・デフォルト）/ `mincho`（明朝体）",
            ].join("\n"),
            inline: false,
          },
          {
            ...
            value: [
              ...
              "",
              "**表示行数の目安**（縮小なし）",
              "big: 約7行 / medium: 約12行 / small: 約21行",
              "超えた場合は自動縮小されます。解像度に合わせて動的に調整されます。",
            ].join("\n"),
            inline: false,
          },
        );
        ...
        .setFields(
          ...
          },
          {
            ...
            value: [
              ...
              "`?mincho red? 明朝体テキスト` → 明朝体・赤色・流れる",
              "`?gothic small shita? 小さいゴシック` → ゴシック体・小・下固定",
            ].join("\n"),
            inline: false,
          },
        );
